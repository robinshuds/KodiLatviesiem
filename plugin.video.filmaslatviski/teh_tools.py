...
if __settings__.getSetting("debug-enable").lower() == "true":debugging=True				#if (debugging==True): 
...
def httpHEADER(url,headers={}):
	#try:
		req=HeadRequest(url)
		req.add_header('User-Agent',net2._user_agent)
		try: _accept=net2._accept
...
def nURL(url,method='get',form_data={},headers={},html='',proxy='',User_Agent='',cookie_file='',load_cookie=False,save_cookie=False, login=False):
	if url=='': return ''
	if not login:
	...
	if not GitData['date']==LastData['date']:
		if not GitData['message']==LastData['message']:
			if not GitData['message']=='-': 
				zz=['KA ',ps("__plugin__")+' ','Kiss '+ps('common_word')+' ',ps('common_word')+' ','HUB-HUG ','HUBHUG ','HUB HUG ']; zzz=0
				for z in zz:
...
def do_My_Splash(HowLong=10): #HowLong in seconds.
	...
	TempWindow2=MyWindowCountDown(bgArt=ps('_domain_url')+'/Content/images/logo.png',L=L,T=T,W=W,H=H); 
...
def ANtoCS(tA):
	#try:
			...
			for i in tA:
				#t+=chr(tA[i])
				#debob([i,tA[i],chr(tA[i])])
				t+=chr(i)
				#debob([i,chr(i)])
			#debob(['ANtoCS()',tA,t])
			return t
	#except: return str(tA)

def KEDecrypt(t):
    #try:  
        testurl = net.url_with_headers(url, cookies=1)
        tcookie = re.findall('Cookie=(.*?)%3B(.*?)%3B(.*?)%3B(.*)%3B',testurl)
        dcookie = ''
        for i in range(4):
            dcookie += tcookie[0][i].replace('%3D','=') + '; '
        dh = {'user-agent': 'Mozilla/5.0 (Linux; Android 4.4.2; Nexus 4 Build/KOT49H) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/34.0.1847.114 Mobile Safari/537.36',
            'Cookie': dcookie,'referer': url}  
        dh.update({'X-Requested-With': 'XMLHttpRequest'})#, 'Content-Length': '0'})
          
        testurl = net.http_POST(url,dh).content        
        js = re.search(r'(?s)type\=(?:\"|\')text\/javascript(?:\"|\')\>(.*?(window[^\=]+).*?)\<\/script\>',testurl)                            
        ra = re.findall(r'(.+(window([^\=]+))[^\;]+)',js.group(1))   
        ...
        #dh.update({'krsk': 'nsNsjn3j'})#ctest})        
        r = net.http_POST('http://kissasian.com/External/RSK',dh).content

        #strmn = '//storage//.kodi//addons//plugin.video.kissasian//testfile.log'
        #file_desc = xbmcvfs.File(strmn, 'w')
        #result=file_desc.write(str(dh))
        #file_desc.close()     
         
        import resources.lib.kissenc as kissenc
        KissDecrypt=kissenc.KissDecrypt()
        
        #return KissDecrypt.decrypt(t,'cartoon') #KissCartoon
        return KissDecrypt.decrypt(t,'drama',r) #KissAsian
    #except: return t
